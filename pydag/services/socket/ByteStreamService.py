...
@dataclass
class ByteStreamService(ReadService, WriteService):
    """
    ...
    def _read_from_source(self):
        ...
        if len(self.addresses) == 0:
            if len(self.get_buffers()) == 1:
                ...
                if len(self.get_buffers()) != len(tn):
                    raise ServiceException("Number of buffers does not match number of data fields")
                for data in data_tuples:
                    b = 0
            ...
            tn = next(iter(data_tuples))
            if len(self.addresses) != len(tn):
                raise ServiceException("Number of addresses does not match number of data fields")             
            if len(self.get_buffers()) == 1:
                buffer = next(iter(self.get_buffers().values()))
                for data in data_tuples:
                    dic = {}
    ...
    def _write_to_sink(self):
        if len(self.addresses) == 0:
            if len(self.get_buffers()) == 1:
                buffer = next(iter(self.get_buffers().values()))    
                d = buffer.data(n = self.n, persistent = self.persistent)
                data_lists = []
                b, f = ByteStreamService.decode_schema(self.write_byte_schema)
                if len(d.keys()) == f:
                    for key in d.keys():
                    ...
                    raise ServiceException("Number of buffer keys does not match number of schema fields")
            else:
                data_lists = []
                for buffer in self.get_buffers().values():
                    d = buffer.data(n = self.n, persistent = self.persistent)
    ...
    def force_schema(schema : str, data : Union[tuple | list[tuple]]) ->  Union[tuple | list[tuple]]:
        ...
        def convert_value(value, fmt):
            if fmt in ['b', 'B', 'h', 'H', 'i', 'I']:
                ...
                raise ServiceException(f"Unknown format code: {fmt}")
        ...
        if isinstance(data, tuple):
            if len(data) != len(formats):
                raise ServiceException("Data length does not match schema length")
            return tuple(convert_value(data[i], formats[i]) for i in range(len(data)))
        elif isinstance(data, list):
            converted_list = []
            for item in data:
                if len(item) != len(formats):
                    raise ServiceException("Data item length does not match schema length")
                converted_item = tuple(convert_value(item[i], formats[i]) for i in range(len(item)))
                converted_list.append(converted_item)
            return converted_list
        else:
            raise ServiceException("Data must be a tuple or list of tuples")
        ...
        if isinstance(data, tuple):
            ...
            for item in data:
                if isinstance(item, tuple):
                    ...
                    raise ServiceException("Data items must be tuples when data is a list")
    ...
    def _decode(self, schema : str, data : bytes) -> list[tuple]:
        ...
        """
        sb, fn = ByteStreamService.decode_schema(schema)
        if len(data) != sb:
            if len(data) % sb == 0:
                values = [
                ...
                raise ServiceException(f"Data length {len(data)} does not match schema size {sb}")
        else:
