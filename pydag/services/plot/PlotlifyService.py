...
class PlotlifyService(Service):
    ...
    def create_plotly_doc(self, buffer_id, data : list[dict], layout : dict, plot_path : str) -> PlotlyDocument:
        ...
        if self._agent is not None:
            if buffer_id in self._agent.get_buffer_store():
                d = self._agent.get_buffer(buffer_id).data()
    ...
    @staticmethod
    def surface(x : np.ndarray = None,
             y : np.ndarray = None, 
             z : np.ndarray = None,
             name : str = "trace1", title : str = "Plotly", x_label : str = "x", y_label : str = "y", z_label : str = "z", color : Union[Color | str] = None) -> PlotlyDocument:
        """
            creates a 3D mesh plot `PlotlyDocument`
        """
        if x is None or y is None or z is None:
            logger.error("No complete x, y, z data was specified")
            return
        p = Plotly()
        p.get_layout().get_title().set_text(title)
        p.get_layout().get_scene().get_xaxis().set_title(x_label)
        ...
        t = Trace()
        t.set_name(name).set_type(PlotType.SURFACE)
        t.set_x(x).set_y(y).set_z(z)
        t.wireframe()
        if color:            
            color_list = [[0, str(color)], [1, str(color)]]
            t.set_colorscale(color_list)
            sc = np.zeros_like(z).tolist()
            t.set_surfacecolor(sc)
    ...
    def mesh3d(x : np.ndarray = None,
        ...
        t = Trace()
        t.set_name(name).set_type(PlotType.MESH3D)
        t.set_x(x).set_y(y).set_z(z)
