...
class LLMSQLService(LLMService):
    ...
    def start(self):
        ...
        self.db = SQLDatabase.from_uri(self.sql_connection)
        self._create_llm()
        self.workflow = StateGraph(State)
        self.workflow.add_node(WRITE_QUERY, self._write_query)
        self.workflow.add_node(EXECUTE_QUERY, self._execute_query)
        self.workflow.add_node(GENERATE_ANSWER, self._generate_answer)
        self.workflow.add_edge(START, WRITE_QUERY)
    ...
    def _write_query(self, state: State):
        ...
        prompt = self.messages.invoke(
            {
                ...
                "top_k": 10,
                "table_info": self.db.get_context(),
        ...
        if self.model_provider == ModelProvider.OLLAMA.value:
            ...
            self.structured_llm = self.llm.with_structured_output(QueryOutput)
