...
from pydag.agents.RuntimeStorage import ArtifactPolicy, normalize_runtime_key
...
class RAGService(LLMService):
    ...
    NON_TEXT_EXTENSIONS: ClassVar[set[str]] = FileUtils.COMMON_NON_TEXT_EXTENSIONS | {".db", ".sqlite", ".sqlite3"}
    ...
    _resolved_vector_store_directory : str | None = artifact_descriptor_field(
        default=None,
        init=False,
        repr=False,
        name="vector_store",
    ...
    def rebuild_runtime_handles(self, agent=None):
        ...
        if (
            ...
            and self._langchain is not None
        ):
            return
        ...
        resolved_directory = self._resolve_vector_store_directory(runtime_agent)
    ...
    def _resolve_vector_store_directory(self, agent=None):
        if self._resolved_vector_store_directory:
            return self._resolved_vector_store_directory
        AgentConfig.ensure_resource_layout()
        ...
        if configured_path is None:
            shared_directory = AgentConfig.EMBEDDING_RESOURCE_ROOT / normalize_runtime_key(
                self.uid, fallback="vector-store"
            )
            FileUtils.create_dir(str(shared_directory))
            return str(shared_directory)

        configured = Path(str(configured_path))
        if not configured.is_absolute():
            configured = AgentConfig.EMBEDDING_RESOURCE_ROOT / configured

        configured_path = os.path.normpath(str(configured))
        ...
        if basename in RAGService.CHROMA_DB_FILENAMES or extension in {".db", ".sqlite", ".sqlite3"}:
            ...
            FileUtils.create_dir(target)
            return target

        if os.path.isdir(configured_path):
            FileUtils.create_dir(configured_path)
    ...
    def restore_state(self, payload: dict | None):
        ...
        if payload is not None and "document_links" in payload:
            self.document_links = list(payload["document_links"])
    ...
    def _is_non_text_document(self, document_link: str) -> bool:
        ...
        return extension in self.NON_TEXT_EXTENSIONS
