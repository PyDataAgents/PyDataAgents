...
class DatasetBuffer(DictBuffer):
    ...
    def _on_install(self, agent : Agent = None):
        ...
        # adjust capacity to the exact size of dataset
        key, val = next(iter(data.items()))
        self.capacity = len(val)

        # IMPORTANT: use _on_push instead of push here.
        # Reason: During installation duplicates are already created and their own
        # install() method (for the same DatasetBuffer type) has loaded the dataset
        # into themselves. Calling push() would forward the data again to each
        # duplicate causing the duplicates to contain the dataset twice.
        # Using _on_push inserts data only into this buffer, preventing double loads.
        # This also allows to later use push() for adding new data without duplicating it in duplicates.
        # Alternative would be here to use push() but additionally have a "loaded" flag to prevent pushing to duplicates during install.
