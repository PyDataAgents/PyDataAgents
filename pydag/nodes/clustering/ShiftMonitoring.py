...
@dataclass
class ShiftMonitoring(LearningElement):

    """
    ShiftMonitoring is a LearningElement that monitors distributional shifts in incoming data. It uses statistical methods to compare the distribution of new data against learned distributions and can identify when a significant shift occurs.
    ...
    def learn(self, data : dict, meta : dict = None) -> bool:
        ...
        for i, key in enumerate(data.keys()):
            self.models[key] = DistributionMonitoring(self.inference_buffer_size, self.bin_count, self.compare_mode, self.n_distributions, self.return_input, key, self.sensitivity) # Add a new model
            d = np.array(data[key])
            if d.ndim == 1:
...
class DistributionA():
    ...
    def __init__(self, A, initialization_edges=None, initialization_KDTree=None,  compare_mode = None, bin_count = None, bins= None):
        ...
        for dim in range(self.num_dims):
            ...
            if std == 0:
                self.bin_edges = self.initialization_edges
            else:
