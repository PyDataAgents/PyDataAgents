...
@dataclass
class RIFEExtractor(LearningElement):
    ...
    def install(self, agent: Agent = None):  # type: ignore[override]
        super().install(agent)
        # Model attribute kept for symmetry with other extractors
        self.model = self._ri_fe

    # LearningElement abstract requirements
    ...
    def infer(self, data: dict, meta: dict | None = None) -> Tuple[Dict, Dict]:  # noqa: D401
        ...
        for i, key in enumerate(data.keys()):
            ...
            forecast[key + "-" + DataElementConfig.FEATURE + f"-rife-{i}"] = feats.tolist()
