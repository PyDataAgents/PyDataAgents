...
@dataclass
class IsomapDimReduction(LearningElement):
    ...
    def learn(self, data : dict, meta : dict = None) -> bool:
        for i, key in enumerate(data.keys()):
            self.models[key] = Isomap(n_components=self.dimensions) # Add a new model
            d = np.array(data[key])
            if d.ndim == 1:
    ...
    def infer(self, data : dict, meta : dict = None) -> Tuple[Dict, Dict]:
        ...
        for i, key in enumerate(data.keys()):
            ...
            forecast[key + "-" + DataElementConfig.FEATURE + f"-ISOMap"] = transformed_data.tolist()  #convert to list
