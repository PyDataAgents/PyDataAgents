...
class ScriptTransform(Transform):
    ...
    def install(self, agent : Agent = None):
        ...
        if self.script_file:
            ...
            raise DataElementException("Script file path must be provided.")
        
    def transform(self, data : dict) -> dict:
        if not self.code:
            raise DataElementException("Script code is not loaded. Please call install() first.")
        ...
        try:
            ...
            raise DataElementException(f"Error executing script: {e}") from e
