...
class HuggingFaceNode(BufferNode, Action):
    ...
    by_rows : bool = field(default=True, metadata={"description": "specifies whether data is retrieved by rows (True) or as columns (False)"})
    ...
    def _on_execute(self):
        ...
        if isinstance(data, dict):
            if len(data) == 1 and len(self.output_keys) == 0:
                new_data = self._pipeline(next(iter(data.values())))
                self.add_data(new_data)
            elif len(self.output_keys) > 0 and len(self.output_keys) == len(data):
