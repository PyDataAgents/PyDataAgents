...
@dataclass
class LLMScriptElement(LearningNode, ServiceNode):
    ...
    def _on_install(self, agent : Agent = None):
        LearningNode._on_install(self, agent)
        ServiceNode._on_install(self, agent)
        if not isinstance(self._service, LLMService):
            raise NodeException("referenced service is not an instance of " + LLMService.cname())
        
    def _on_uninstall(self, agent : Agent = None):
        LearningNode._on_uninstall(self, agent)
        ServiceNode._on_uninstall(self, agent)          
    ...
    def learn(self, data : dict, meta : dict = None) -> bool:
        ...
        if isinstance(self._service, LLMService):
            self._code = self._service.chat(self.human_msg)
            if self._is_valid_python(self._code):
                return False
            else:
                raise NodeException("the generated code is not valid Python code")
        else:
            raise NodeException("referenced service is not an instance of " + LLMService.cname())
    ...
    def infer(self, data : dict, meta : dict = None) -> Tuple[Dict, Dict]:
        ...
        """
        self._run_generated_code(self._code, data)
