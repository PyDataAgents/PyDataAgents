...
class AdapterReadAction(AdapterNode, Action):
    ...
    def _on_execute(self):
        ...
        if isinstance(self._adapter, ReadAdapter):
            self._adapter.read_from_source(buffer_dict, address_list, self.n)
        else:
            raise NodeException(f"Adapter {self._adapter.id} is not a ReadAdapter.")
