...
@dataclass
class RegressionTransform(LearningElement):
    ...
    def install(self, agent : Agent = None):
        ...
        if self.model_name == "Tirex":
            ...
            self.model : ChronosForecaster = ChronosForecaster(model_path="amazon/chronos-t5-tiny")
    ...
    def infer(self, data : dict, meta : dict = None) -> Tuple[Dict, Dict]:
        if self.model_name == "Tirex":
            ...
            for key, d in data.items():
                ...
                fc = self.model.forecast(context=x, prediction_length=self.prediction_length, output_type="numpy")[1] # mean
                for i in range(fc.shape[0]):
                    forecast[key + "-" + DataElementConfig.FEATURE + f"-{self.model_name}-{i}"] = fc[i].tolist()  #convert to list
            ...
            for i, key in enumerate(data.keys()):
                self.model.fit(y=np_data[i], fh=fh)
                y_pred = self.model.predict()
                forecast[key + "-" + DataElementConfig.FEATURE + f"-{self.model_name}-{i}"] = y_pred.tolist()  #convert to list
            return forecast, None
