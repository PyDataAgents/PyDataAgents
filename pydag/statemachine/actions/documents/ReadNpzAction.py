...
class ReadNpzAction(BufferNode, Action):
    ...
    def execute(self):
        if FileUtils.exists_file(self.file_path):
            if isinstance(self.buffer, DictBuffer):
                data = np.load(self.file_path)
                d = {key: DataUtils.ndarray_to_list(data[key]) for key in data.files}
                data.close()
                self.buffer.push(d)
            else:
                raise StatemachineException("only " + Buffer.cname() + "s of type " + DictBuffer.cname() + " are allowed")
        else:
            raise StatemachineException("filepath " + self.file_path + " does not exist")