...
@dataclass
class WebcamAdapter(ReadAdapter):
    ...
    def _on_disconnect(self) -> bool:
        self._vc.release()
        return not self._vc.isOpened()
    
    def _on_read(self, buffers : dict[str, Buffer], addresses : list[str] = None, n : int = 0):
        if len(buffers) > 1:
            raise AdapterException(f"only 1 {Buffer.cname()} can be used for data storage")
        else:
            ...
            if self.encode_base64:
                ...
                if success:
                    ...
                    raise AdapterException(f"could not encode image as base64 string in {WebcamAdapter.cname()}")
            else:
    ...
    def _autodetect_resolution(self):
        ...
        if w > 0 and h > 0:
            ...
            raise AdapterException("Could not autodetect frame resolution")        
