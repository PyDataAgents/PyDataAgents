...
| `instruction_key` | `str` | `` | Key from which to receive the Rules on how to answer (format, style, constraints, priorities). |
| `instruction_value` | `str` | `` | Fixed value for the Rules on how to answer (format, style, constraints, priorities). |
...
| `context_files_value` | `str | bytes | list[str | bytes] | None` | `` | Fixed optional file context for all rows. V1 accepts external URLs, file URLs, absolute local paths, data URIs, plain base64 strings, raw bytes, or lists of those values. |
