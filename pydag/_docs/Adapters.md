...
|-------|-------------|------|
| [`Adapter`](#adapter-in-pydagadaptersadapterpy) | Abstract base class for `Adapters`. All `Adapters` must inherit from this class. | ![Adapter](element_icons/Adapter.png)
| [`PublishAdapter`](#publishadapter-in-pydagadapterspublishadapterpy) | abstract class for `Adapter` Interface for publishing to data sinks.<br>new `Adapters` that allow for publishing to a sink via callback must inherit this class next to `Adapter`. | ![PublishAdapter](element_icons/PublishAdapter.png)
| [`ReadAdapter`](#readadapter-in-pydagadaptersreadadapterpy) | abstract class for `Adapter` Interface for reading from data sources.<br>new `Adapters` that allow for reading from a source via one-shot polling must inherit this class next to `Adapter`. | ![ReadAdapter](element_icons/ReadAdapter.png)
| [`SubscribeAdapter`](#subscribeadapter-in-pydagadapterssubscribeadapterpy) | abstract class for `Adapter` Interface for subscribing from data sources<br>new `Adapters` that allow for subscribing to a source via callback must inherit this class next to `Adapter`. | ![SubscribeAdapter](element_icons/SubscribeAdapter.png)
| [`WriteAdapter`](#writeadapter-in-pydagadapterswriteadapterpy) | abstract class for Adapter Interface for writing to data sinks<br>new `Adapters` that allow for writing to a sink via one-shot polling must inherit this class next to `Adapter`. | ![WriteAdapter](element_icons/WriteAdapter.png)
| [`AdsAdapter`](#adsadapter-in-pydagadaptersadsadsadapterpy) | `Adapter` for reading and writing data from/to Beckhoff TwinCAT PLCs via ADS (Automation Device Specification).     | ![AdsAdapter](element_icons/AdsAdapter.png)
| [`AudioAdapter`](#audioadapter-in-pydagadaptersaudioaudioadapterpy) | `Adapter` for subscribing to a system's audio input channels (e.g. from a USB microphone) using the `sounddevice` library.     | ![AudioAdapter](element_icons/AudioAdapter.png)
| [`SolidPDMAdapter`](#solidpdmadapter-in-pydagadapterscadsolidpdmadapterpy) |  | ![SolidPDMAdapter](element_icons/SolidPDMAdapter.png)
| [`CsvReadAdapter`](#csvreadadapter-in-pydagadapterscsvcsvreadadapterpy) | `Adapter` for reading data from CSV files.     | ![CsvReadAdapter](element_icons/CsvReadAdapter.png)
| [`CsvWriteAdapter`](#csvwriteadapter-in-pydagadapterscsvcsvwriteadapterpy) | `Adapter` for writing data to CSV files.     | ![CsvWriteAdapter](element_icons/CsvWriteAdapter.png)
| [`InfluxDbAdapter`](#influxdbadapter-in-pydagadaptersdbinfluxdbadapterpy) | `Adapter` thats reads or writes to InfluxDB.<br>Address Schema:<br>address = "b=[bucket];m=[measurement];f=[field]" | ![InfluxDbAdapter](element_icons/InfluxDbAdapter.png)
| [`SQLAdapter`](#sqladapter-in-pydagadaptersdbsqladapterpy) | `Adapter` for reading and writing data from/to SQL databases using pyodbc.<br>Required ODBC driver must be installed for the specific SQL database (e.g. MySQL, PostgreSQL, SQLite, etc.) and system<br><br>The `addresses` in `_on_write` and `_on_read` is used to specify the SQL statement to execute. SQL statements should adhere the following format:- SELECT: 'SELECT [column1], [column2], ... FROM [table] WHERE [column3] = [value1]'- CREATE TABLE: 'CREATE TABLE ...'        - INSERT: 'INSERT INTO [table] ([column1], [column2], [column3], ...) VALUES (?, ?, ?, ...)'- UPDATE: 'UPDATE [table] SET [column1] = ?, [column2] = ?, ... WHERE [column3] = ?'- DELETE: 'DELETE FROM [table] WHERE [column1] = ? AND [column2] = ? OR ...'- ALTER: 'ALTER TABLE [table] ADD COLUMN [column1] [datatype] {DEFAULT [value]}'- DROP: 'DROP TABLE [table]' | ![SQLAdapter](element_icons/SQLAdapter.png)
| [`DocumentTextAdapter`](#documenttextadapter-in-pydagadaptersdocumentsdocumenttextadapterpy) | `Adapter` that retrieves text content from specified files         | ![DocumentTextAdapter](element_icons/DocumentTextAdapter.png)
| [`DocxAdapter`](#docxadapter-in-pydagadaptersdocumentsdocxadapterpy) | `Adapter` for writing data to DOCX documents.The specified addresses in `write_to_sink` can be used to map data keys from buffer to place holders in word template.If no addresses are specified all buffer keys are directly mapped to the context of the word template | ![DocxAdapter](element_icons/DocxAdapter.png)
| [`NpzAdapter`](#npzadapter-in-pydagadaptersdocumentsnpzadapterpy) | `Adapter` that retrieves data from a *.npz numpy file         | ![NpzAdapter](element_icons/NpzAdapter.png)
| [`HttpAdapter`](#httpadapter-in-pydagadaptershttphttpadapterpy) | `Adapter` for reading and writing data from/to http endpoints     | ![HttpAdapter](element_icons/HttpAdapter.png)
| [`MQTTAdapter`](#mqttadapter-in-pydagadaptersmqttmqttadapterpy) | `Adapter` for subscribing or writing data from/to MQTT topics.     | ![MQTTAdapter](element_icons/MQTTAdapter.png)
| [`OpcUaAdapter`](#opcuaadapter-in-pydagadaptersopcuaopcuaadapterpy) | `Adapter` for reading and writing data from/to OPC UA servers.     | ![OpcUaAdapter](element_icons/OpcUaAdapter.png)
| [`S7Adapter`](#s7adapter-in-pydagadapterss7s7adapterpy) | `Adapter`reading from and writing to S7 PLCs.     | ![S7Adapter](element_icons/S7Adapter.png)
| [`ByteStreamAdapter`](#bytestreamadapter-in-pydagadapterssocketbytestreamadapterpy) | `Adapter` to read and write byte streams from/to a socket connection.<br>The adapter can be configured with different byte schemas for connecting, disconnecting,sending, and receiving data.<br>The bytescheams are defined as a string of data types, e.g. "Bhf5s" -> uint8, int16, float32, string of length 5<br>The addresses in read_from_source and write_to_sink are used to specify the buffer keys to read from or write to.<br>e.g. addresses = ["B1", "B3", "SENSOR1"]<br>The length of the addresses list must not match the number of buffers passed, all buffers are being searched for the keys in addresses.But it has to match the number of elements in the schema used for reading or writing. Omiting schema fields can be done by specifying None in the addresses list.<br>For Example:<br>schema = "BfI" -> addresses = ["ID1", None, "ID3"] | ![ByteStreamAdapter](element_icons/ByteStreamAdapter.png)
| [`SerialAdapter`](#serialadapter-in-pydagadapterssocketserialadapterpy) | `Adapter` for serial communication using pySerial.     | ![SerialAdapter](element_icons/SerialAdapter.png)
| [`TCPClientAdapter`](#tcpclientadapter-in-pydagadapterssockettcpclientadapterpy) |  | ![TCPClientAdapter](element_icons/TCPClientAdapter.png)
| [`WebSocketAdapter`](#websocketadapter-in-pydagadapterssocketwebsocketadapterpy) | `Adapter` for subscribing and writing data from/to WebSocket endpoints.     | ![WebSocketAdapter](element_icons/WebSocketAdapter.png)
| [`VSEAdapter`](#vseadapter-in-pydagadapterssocketifmvsevseadapterpy) |  | ![VSEAdapter](element_icons/VSEAdapter.png)
| [`WebcamAdapter`](#webcamadapter-in-pydagadaptersvisionwebcamadapterpy) | An `Adapter` that captures webcam video feed into a `Buffer`     | ![WebcamAdapter](element_icons/WebcamAdapter.png)
