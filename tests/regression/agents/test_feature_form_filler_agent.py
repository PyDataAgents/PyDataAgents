...
def _build_instruction() -> str:
    return (
        ...
        "Use information from retrieved context documents.\n"
        "The input context contains the form fields to fill (including field_id and optional label_context).\n"
        "Infer the best field-to-value mapping from that context.\n"
        "Output format must be: {\"<field_id>\": \"<value>\", ...}.\n"
...
def test_feature_form_filler_agent_end_to_end_local_example():
    ...
    llm_fill_action = LLMChatAction(
        ...
        question_value="Fill the PDF form fields from the retrieved context.",
        instruction_value=_build_instruction(),
        retrieval_query_key="fields",
        input_context_keys=["fields", "metadata", "full_text_content"],
    ...
    try:
        ...
        action_service.get_observer_thread()._thread.join(timeout=180)
