...
def test_feature_form_filler_agent_quality_context2_openai_generated_question_retrieval():
    """Quality benchmark: context_2-only RAG source + per-field expected answer comparison."""
    config = configparser.ConfigParser()
    config.read("config.ini")
    if not config.has_section("OPENAI"):
        pytest.skip("Skipping OpenAI quality benchmark: missing [OPENAI] section in config.ini")
    openai_api_key = config.get("OPENAI", "OPENAI_API_KEY", fallback="").strip()
    if openai_api_key == "":
        pytest.skip("Skipping OpenAI quality benchmark: missing OPENAI_API_KEY in config.ini")

    context2_source = _rag_context_folder() / "context_2.txt"
    if not context2_source.exists():
        pytest.skip("Skipping OpenAI quality benchmark: context_2.txt not found")

    expected = _context2_expected_field_answers()
    target_accuracy = float(os.environ.get("FORM_FILLER_TARGET_ACCURACY", "1.0"))

    with tempfile.TemporaryDirectory(prefix="rag_context2_only_") as temp_context_dir:
        temp_context_path = Path(temp_context_dir) / "context_2.txt"
        temp_context_path.write_text(context2_source.read_text(encoding="utf-8"), encoding="utf-8")

        embedding_store_name = "form_filler_quality_context2_openai_" + str(os.getpid())
        embedding_service = FileEmbeddingService(
            id="FILE_EMBEDDING_SERVICE_OPENAI_QUALITY",
            docs_folder=str(temp_context_dir),
            store_name=embedding_store_name,
        )
        ...
        list_files_action = ListFilesAction(
            id="LIST_PDFS_OPENAI_QUALITY",
            folder=str(_pdf_form_folder()),
            extension=".pdf",
        )

        read_pdf_form_action = PDFReadFormAction(
            id="READ_PDF_FORM_OPENAI_QUALITY",
            input_keys=["values"],
            row_mode="per_field",
            include_bridge_prompt=True,
        )
        ...
        llm_fill_action = LLMChatAction(
            id="LLM_FILL_FORM_OPENAI_QUALITY",
            question_key="llm_prompt",
            instruction_value=_build_instruction(),
        ...
        action_service = SimpleActionService(
            id="FORM_FILLER_ACTION_SERVICE_OPENAI_QUALITY",
            thread_type=ThreadType.ONLY_ONCE.value,
        )
        action_service.add_node(list_files_action)
        action_service.add_node(read_pdf_form_action)
        ...
        agent = Agent(id="FORM_FILLER_AGENT_E2E_OPENAI_QUALITY")
        agent.add_service(embedding_service)
        agent.add_service(rag_service)
        agent.add_service(action_service)
        ...
        try:
            ...
            if "answer" not in llm_data or len(llm_data.get("answer", [])) == 0:
                ...
                pytest.skip(
                    "Skipping OpenAI quality benchmark: no LLM answer produced "
                    + "(state="
                    + state_label
            ...
            write_data = write_pdf_form_action.get_buffer().data()
            written_rows = write_data.get("written_fields", [])
            assert len(written_rows) > 0
            written_fields = written_rows[0]
            actual = {str(key): _normalize_pdf_value(value) for key, value in written_fields.items()}

            missing_fields = [field_id for field_id in expected.keys() if field_id not in actual]
            mismatches = [
                (
                    field_id,
                    expected[field_id],
                ...
                if field_id in actual and _normalize_pdf_value(actual[field_id]) != _normalize_pdf_value(expected[field_id])
            ]

            matched_count = len(expected) - len(missing_fields) - len(mismatches)
            accuracy = float(matched_count) / float(len(expected))
            print("context_2 quality benchmark accuracy:", f"{accuracy:.3f}")
            print("context_2 quality benchmark matched:", matched_count, "/", len(expected))
            if len(missing_fields) > 0:
                print("context_2 quality benchmark missing fields:", ", ".join(sorted(missing_fields)))
            if len(mismatches) > 0:
                preview = mismatches[:15]
                print("context_2 quality benchmark mismatches (first 15):", json.dumps(preview, ensure_ascii=True))

            assert accuracy >= target_accuracy, (
                "context_2 quality benchmark accuracy "
                + f"{accuracy:.3f}"
                + " is below target "
