...
def test_clear_and_to_dict_and_str_json():
    ...
    assert AgentKeywords.DATA in parsed and AgentKeywords.META in parsed
    assert parsed[AgentKeywords.META]["description"] == "desc"
    buf.clear()
    assert buf.size() == 0
...
def test_duplicate_ids_without_agent():
    ...
    dup1_vals = buf.get_duplicates()["dup1"].data()[AgentKeywords.VALUES]
    dup2_vals = buf.get_duplicates()["dup2"].data()[AgentKeywords.VALUES]
