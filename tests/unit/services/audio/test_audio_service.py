...
from pydag.services.audio.AudioService import AudioService
...
def test_020():
    audio = AudioService(id="A1", sample_rate=16_000)
    audio.install()
...
def test_031():
    ...
    audio = AudioService(id = "A1", sample_rate = sr)
    audio.add_buffer(buf)
    audio.install()
