...
## Technical Reference
For detailed low-level mechanics of runtime persistence, checkpoints, exact-match auto-resume, auto-checkpoint behavior, cleanup, declarative element persistence, runtime-handle reconstruction, and the current resilient single-agent lifecycle, see [TECHNICAL_REFERENCE.md](TECHNICAL_REFERENCE.md).
...
<br>The newer runtime persistence and restart details, including deterministic runtime identities, declarative checkpoint field roles, runtime-handle reconstruction, exact-match auto-resume, runtime storage paths, cleanup policies, base-class persistence responsibilities, and a guide for building new elements, are documented in [TECHNICAL_REFERENCE.md](TECHNICAL_REFERENCE.md).
